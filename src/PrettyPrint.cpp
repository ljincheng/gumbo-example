...
static std::string refuse_inline   = "|script|noscript|iframe|style|";//拒绝不安全的元素
...
static std::string build_img_attributes(GumboAttribute * at, bool no_entities,const char * baseUrl)
{
    std::string atts = "";
    ...
    if ( (!std::string(at->value).empty())   ||
        ...
        if(strcasecmp(at->name,"src")== 0 && baseUrl !=NULL)
        {
            if (no_entities) {
                const char * _value = at->value;
                atts.append(httpHelper.addOrRemoveBaseHelper(_value, baseUrl));
            } else {
                std::string  _value =substitute_xml_entities_into_attributes(quote, std::string(at->value));
                atts.append(httpHelper.addOrRemoveBaseHelper(_value.c_str(), baseUrl));
            }
        }else{
            if (no_entities) {
                atts.append(at->value);
...
static std::string prettyprintWithBaseUrl(GumboNode*, int lvl, const std::string indent_chars,const char * baseUrl);


// prettyprint children of a node
// may be invoked recursively

static std::string prettyprint_contents(GumboNode* node, int lvl, const std::string indent_chars,const char * baseUrl) {
    ...
    for (unsigned int i = 0; i < children->length; ++i) {
        ...
        if (child->type == GUMBO_NODE_TEXT) {
            ...
            std::string val = prettyprintWithBaseUrl(child, lvl, indent_chars,baseUrl);
...
static std::string prettyprint(GumboNode* node, int lvl, const std::string indent_chars) {
    ...
    if (node->type == GUMBO_NODE_DOCUMENT) {
        ...
        results.append(prettyprint_contents(node,lvl+1,indent_chars,NULL));
        return results;
    }
    ...
    std::string contents = prettyprint_contents(node, lvl+1, indent_chars,NULL);
   ...
   // results.append("<"+tagname+atts+close+">");
    if(strcasecmp(tagname.c_str(),"img")== 0)
    {
        results.append("<"+tagname+atts+close+">");
    }else{
        results.append("<"+tagname+close+">");
    }
...
static std::string prettyprintWithBaseUrl(GumboNode* node, int lvl, const std::string indent_chars,const char * baseUrl) {
    
    // special case the document node
    if (node->type == GUMBO_NODE_DOCUMENT) {
        std::string results = build_doctype(node);
        results.append(prettyprint_contents(node,lvl+1,indent_chars,baseUrl));
        return results;
    }
    ...
    bool is_inline                 = nonbreaking_inline.find(key) != std::string::npos;
    bool inline_like               = treat_like_inline.find(key) != std::string::npos;
    bool refuse_tag                = refuse_inline.find(key) != std::string::npos;
    bool pp_okay                   = !is_inline && !keep_whitespace;
    char c                         = indent_chars.at(0);
    int  n                         = indent_chars.length();
    ...
    for (int i=0; i< attribs->length; ++i) {
        ...
        if(strcasecmp(tagname.c_str(),"img")== 0)
        {
            atts.append(build_img_attributes(at, no_entity_substitution,baseUrl));
        }else{
            atts.append(build_attributes(at, no_entity_substitution));
        }
    }
    ...
    // prettyprint your contents
    std::string contents = prettyprint_contents(node, lvl+1, indent_chars,baseUrl);
...
PrettyPrint::PrettyPrint(GumboNode* apNode)
{
    mpNode=apNode;
}
...
std::string PrettyPrint::prettyPrintNode()
{
    
    std::string indent_chars = "  ";
    return prettyprint(mpNode,1,indent_chars);
}


std::string PrettyPrint::prettyPrintNode(const char * baseUrl){
    std::string indent_chars = "  ";
    return prettyprintWithBaseUrl(mpNode,1,indent_chars,baseUrl);
}
